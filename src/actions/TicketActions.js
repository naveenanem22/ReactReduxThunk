...
export function modalMessageAndUpdateTicket() {
    return {
        type: MODAL_MESSAGE_UPDATE_TICKET
    }
}

export function modalMessageAndUpdateTicketSuccess() {
    return {
        type: MODAL_MESSAGE_UPDATE_TICKET_SUCCESS
    }
}

export function modalMessageAndUpdateTicketFailure() {
    return {
        type: MODAL_MESSAGE_UPDATE_TICKET_FAILURE
    }
}
...
export function modalMessageAndUpdateTicketAPICall(params, queryParams) {
    console.log("params:");
    console.log(params);
    ...
    Object.keys(queryParams).forEach(key => url.searchParams.append(key, queryParams[key]));
    return function (dispatch) {
        dispatch(modalMessageAndUpdateTicket());
        return fetch(url, {
            method: 'PUT',
            headers: headers,
            body: formData
            ...
            .then(
                response => {
                    if (response.status === 204) {
                        dispatch(modalMessageAndUpdateTicketSuccess());
                    }
                    if (response.status === 500 || response.status === 400) {
                        console.log("Dispatching close ticket failure");
                        dispatch(modalMessageAndUpdateTicketFailure());
                    }
